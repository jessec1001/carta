...
const SchemaArrayInput: FunctionComponent<SchemaArrayInputProps> = ({
  ...
  // We have to add a small hack here to disable the component briefly after being interacted with.
  // Otherwise, the overlap of some component can cause an event to trigger multiple changes.
  const [disabled, setDisabled] = useState(false);
  useEffect(() => {
    // Immediately enable the component modifications after the click event is no longer propagating.
    setDisabled(true);
    const timeoutId = setTimeout(() => setDisabled(false), 0);
