...
/** The props used for the {@link EditorPaletteItem} component. */
interface EditorPaletteItemProps extends ComponentProps<"div"> {
...
const EditorPaletteItem: FC<EditorPaletteItemProps> = ({
  type,
  describe,
  onClickTag,
  ...props
...
/**
 * The props used for the {@link EditorPalette} component.
 */
interface EditorPaletteProps extends ComponentProps<"div"> {
  ...
  onPick?: (option: { type: string; subtype: string | null } | null) => void;
}
 ...
 */
const EditorPalette: FC<EditorPaletteProps> = ({
  position,
  onPick: onSelect,
  ...props
}) => {
  // We need the operations API to request the operation types.
  const { operationsAPI } = useAPI();
  const [types, setTypes] = useState<OperationType[] | null>(null);

  // We check if we are in a workspace. If so, we ammend the requests to include the workspace.
  const workspaceContext = useContext(WorkspaceContext);
  const workspace = workspaceContext?.workspace;

  // We use a query to filter the operations that are relevant to a search.
  // We collect the operation types that are reflective of the query.
  ...
  const requesterDelay = useDelayCallback(requester, 100);
  useEffect(() => {
  ...
  }, [requesterDelay, query, operationsAPI, workspace?.id]);
