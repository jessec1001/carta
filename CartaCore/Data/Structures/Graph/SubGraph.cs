...
namespace CartaCore.Data
{
    ...
    /// </summary>
    public class SubGraph : Graph, IEntireGraph
    {
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets the underlying graph.
        /// </summary>
        /// <value>
        /// <c>null</c> if the graph has no parent graph. Otherwise, refers to the parent graph used to create this
        /// subgraph.
        /// </value>
        ...
        /// <summary>
        /// Initializes an instance of the <see cref="SubGraph"/> class with the specified identifier, properties,
        ...
        /// <summary>
        /// Initializes an instance of the <see cref="SubGraph"/> class with the specified identifier and edge
        /// directionality.
        /// </summary>
        /// <param name="id">The graph identifier.</param>
        /// <param name="directed"><c>true</c> if the edges are directed; otherwise, <c>false</c>.</param>
        /// <param name="dynamic"><c>true</c> if the graph is dynamic; otherwise, <c>false</c>.</param>
        public SubGraph(Identity id, bool directed = true, bool dynamic = true)
            : base(id)
        {
            Directed = directed;
            Dynamic = dynamic;
        ...
        public static async Task<SubGraph> CreateSubgraph(IDynamicGraph<Vertex> graph, IEnumerable<Identity> ids)
        {
            // Copy over the settings from the base graph into a new finite graph.
            SubGraph subgraph;
            if (graph is Graph graphBase)
                subgraph = new SubGraph(graphBase.Identifier, graphBase.Properties, graph.GetProperties().Directed)
                {
                    Label = graphBase.Label,
                    Description = graphBase.Description
                };
            else subgraph = new SubGraph(null, graph.GetProperties().Directed);
