...
namespace CartaCore.Operations
{
    ...
    >
    {
        ...
        private class DescendantsGraph : WrapperGraph, IEntireGraph
        {
            ...
            private readonly IDynamicOutGraph<IVertex, IEdge> DynamicOutGraph;
            private readonly IDynamicInGraph<IVertex, IEdge> DynamicInGraph;
            private readonly HashSet<string> RetrievedIds;
                ...
                : base(graph.Id, graph.Properties)
            {
                ...
                if (((IGraph)graph).TryProvide(out IDynamicOutGraph<IVertex, IEdge> dynamicOutGraph))
                    DynamicOutGraph = dynamicOutGraph;
                if (((IGraph)graph).TryProvide(out IDynamicInGraph<IVertex, IEdge> dynamicInGraph))
                    DynamicInGraph = dynamicInGraph;

                // Setup the storage for unique vertex identities.
                RetrievedIds = new HashSet<string>();
            }
            ...
            private async Task<bool> ContainsVertex(Vertex vertex)
            {
                ...
                RetrievedIds.Add(vertex.Id);

                await foreach (Vertex parentVertex in DynamicInGraph.GetParentVertices(vertex.Id))
                {
                    if (!RetrievedIds.Contains(parentVertex.Id) && await ContainsVertex(parentVertex))
                        return true;
                }
                return false;
            }
            ...
            public async Task<bool> ContainsVertex(IVertex vertex)
            {
                ...
                {
                    RetrievedIds.Clear();
                    IVertex inVertex = await DynamicInGraph.GetVertex(vertex.Id);
                    return await ContainsVertex(inVertex);
                }
                return false;
            }

            private async IAsyncEnumerable<IVertex> TraverseDescendant(IVertex vertex, string id, int? depth)
            {
                ...
                if (!RetrievedIds.Contains(id) && (depth is null || depth > 0))
                {
                    ...
                    RetrievedIds.Add(id);
                    await foreach (IVertex childVertex in DynamicOutGraph.GetChildVertices(id))
                    {
                        // Check if the child vertex has already been retrieved before traversing it.
                        if (RetrievedIds.Contains(childVertex.Id)) continue;

                        await foreach (
                            IVertex descendantVertex in
                            TraverseDescendant(childVertex, childVertex.Id, depth - 1)
            ...
            private async IAsyncEnumerable<IVertex> TraverseRoot(string id)
            {
                ...
                await foreach (IVertex descendantVertex in TraverseDescendant(vertex, id, Depth))
                    yield return descendantVertex;
            }
            ...
            public async IAsyncEnumerable<IVertex> GetVertices()
            {
                ...
                if (DynamicOutGraph is not null)
                {
                    ...
                    RetrievedIds.Clear();
                    await foreach (IVertex vertex in TraverseRoot(Id))
                        yield return vertex;
                }
                else
                {
